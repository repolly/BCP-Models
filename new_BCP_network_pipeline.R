...
for (year in years)
{
  if(year %in% names(syn)[colinds] || year %in% names(syn)[incolinds])
  {
    ...
    a <- data.frame(read.table(paste0("BCP_Fabio/use_",year,".csv"),sep=","))
    names(a) <- a[1,]
    row.names(a) <- a[,1]
    a <- a[-1,]
    a <- a[,-1]
...
cc1 <- aa1-bb1
write.csv(aa1,paste0("avg_CE_use_tables.csv"))
write.csv(bb1,paste0("avg_NOT_CE_use_tables.csv"))
write.csv(cc1,paste0("avg_differences_use_tables.csv"))
