...
for (type in types)
{
  ...
  for (count in counts)
  {
          ...
          for (act_ind in 1:ncol(acts))
          {
            ...
            if(sum(aact!=0)>0)
            {
              ...
              if(zero_counter<=zero_threshhold)
              {
                ...
                ab1$activity_name <- activity
                ab1$extreme <- type
                ab1$country <- count
                ab1$crop_id <- crops_id[ee]
                ab1$crop <- crops[ee]
                ab1$min_coverage <- min_coverage
                ab1$pb <- pb
                ab1$r.squared <- su$r.squared
                ab1$zero_counter <- zero_counter
                ab1$year <- row.names(ab1)
                if(ncol(d1)==ncol(ab1))
                {
                  d1 <- rbind(d1,ab1)
                }else{
                  write.csv(ab1,paste0("lin_mods/",type,"_",count,"_",crops_id[ee],"_",activity,"_lin_mod_with data.csv"))
                }
              }
    ...
    write.csv(d1,paste0("lin_mods/",type,"_",count,"_lin_mod_with data.csv"))
  }
}
names(d0) <- c("country","crop","crop_id", "type", "Scarecity", "activity", "pb","r.squared","zero_counter")
d0
write.csv(d0,"lin_mod_with data.csv")
