...
types1 <- c("heat waves","cold waves", "droughts", "wet extremes")
extr1 <- paste0("Regional exposure_",types1,"_75%")
...
sheetsarea1 <- excel_sheets(paste0("Climate exposure & impact indexes/",extr1[1],".xlsx"))[-29]#29sheets
days1 <- data.frame()
for(j in 1:4)
{
  # j=4
  ...
  {
    # i=4
    days <- data.frame(read_xlsx(paste0("Climate exposure & impact indexes/",extr1[j],".xlsx"),sheet = sheetsarea[i]))
    crop1 <- days$CROP_ID[1]
    extreme <- types1[j]
    days <- days[(133+1):(165-4),]
    days <- days[,c(6:46)]
    days$extreme <- extreme
    names(days)[1] <- "country"
    names(days)[2:41] <- 1981:2020
    days$CROP_ID <- crop1
    days1 <- rbind(days1,days) 
  }
}
days1 <- days1[is.na(days1$CROP_ID)==F,]
days1$CROP_ID[days1$CROP_ID=="C1410"] <- "C1400"
unique(days1$CROP_ID)
days1
...
for(j in 1:4)
{
  ...
  {
    # i=1
    tays <- data.frame(read_xlsx(paste0("Climate exposure & impact indexes/",extr1[j],".xlsx"),sheet = sheetsarea[i]))
    crop1 <- tays$CROP_ID[1]
    extreme <- types1[j]
    tays <- tays[(1):(99),]
    tays <- tays[,c(1,7:46)]
    tays$extreme <- extreme
    names(tays)[2:41] <- 1981:2020
    tays$CROP_ID <- crop1
    tays1 <- rbind(tays1,tays) 
  }
}
tays1 <- tays1[is.na(tays1$CROP_ID)==F,]
tays1$CROP_ID[tays1$CROP_ID=="C1410"] <- "C1400"
tays1$country <- substr(tays$NUTS_ID,1,2)
unique(tays1$CROP_ID)
tays1
############################################create pivort table
piv <- data.frame()
for(i in 1:4){
...
impacts <- data.frame(read_excel(paste0("Climate exposure & impact indexes/EU Climate Extreme Impact database_09.02.2023.xlsx")))
...
for(reg_level in 1:2){
  ...
  for (count in unique(data$country))
  {
    ...
    for(cropid in unique(data$cropid))
    {
      ...
      if(nrow(w)>0 && sum(w[substr(w$NUTS_ID,1,2)==count,3])>0)
      {
        ...
        days2 <- days1[(days1$country==count)+(days1$CROP_ID==cropid)==2,]
        # days1[(days1$country==count),]
        days2 <- days2[is.na(days2$extreme)==F,]
        days2
        nams <- names(dat)[10:17]
        dat$days_heatwave <- as.numeric(t(days2[days2$extreme=="heat waves",2:41][1,]))
        dat$days_coldwave <- as.numeric(t(days2[days2$extreme=="cold waves",2:41][1,]))
        dat$days_drought <- as.numeric(t(days2[days2$extreme=="droughts",2:41][1,]))
        dat$days_flood <- as.numeric(t(days2[days2$extreme=="wet extremes",2:41][1,]))
        ...
        jo$area_drought <- 0
        jo$area_flood <- 0
        tays2 <- tays1[(tays1$country==count)+(tays1$CROP_ID==cropid)==2,]
        tays2 <- tays2[is.na(tays2$extreme)==F,]
        tays2 <- tays2[is.na(tays2$NUTS_ID)==F,]
        for (reg in unique(tays2$NUTS_ID))
        {
          tays2[tays2$NUTS_ID==reg,2:41] <- (tays2[tays2$NUTS_ID==reg,2:41]>0)*jo$share[jo$NUTS_ID==reg][1]
        }
        
        
        dat$exp_area_heatwaves <- 0
        dat$exp_area_coldwaves <- 0
        dat$exp_area_droughts <- 0
        dat$exp_area_floods <- 0
        dat$exp_area_heatwaves[dat$year%in%dat[dat$days_heatwave>0,]$year] <- colSums(tays2[tays2$extreme=="heat waves",names(tays2)%in%dat[dat$days_heatwave>0,]$year])
        dat$exp_area_coldwaves[dat$year%in%dat[dat$days_coldwave>0,]$year] <- colSums(tays2[tays2$extreme=="cold waves",names(tays2)%in%dat[dat$days_coldwave>0,]$year])
        dat$exp_area_droughts[dat$year%in%dat[dat$days_drought>0,]$year] <- colSums(tays2[tays2$extreme=="droughts",names(tays2)%in%dat[dat$days_drought>0,]$year])
        if(count=="FI"||count=="SE")
          {
          dat$exp_area_floods[dat$year%in%dat[dat$days_flood>0,]$year] <- sum(tays2[tays2$extreme=="wet extremes",names(tays2)%in%dat[dat$days_flood>0,]$year])
        }else{
          dat$exp_area_floods[dat$year%in%dat[dat$days_flood>0,]$year] <- colSums(tays2[tays2$extreme=="wet extremes",names(tays2)%in%dat[dat$days_flood>0,]$year])
          }
        names(dat)
  ...
  d0
  write.csv(d0,paste0("BCP_Phase_1_NEW_results_75%_",reg_levels[reg_level],"_Level_90_perc.csv"))
}
# ####EU Level
