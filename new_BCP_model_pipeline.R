...
pivreg <- pivreg[,c(-1,-3,-4,-5,-7:-12,-48:-50,-53)]
pr1 <- pivreg[pivreg$exposure_type==types[1],]
pr2 <- pivreg[pivreg$exposure_type==types[2],]
pr3 <- pivreg[pivreg$exposure_type==types[3],]
pr4 <- pivreg[pivreg$exposure_type==types[4],]
counts <- unique(substr(pr1$NUTS_ID,1,2))
w1 <- sapply(1:nrow(pr1),function(x){max(pr1[x,names(pr1)[3:37]])})
w1[is.na(w1)] <- 0
##
w2 <- sapply(1:nrow(pr2),function(x){max(pr2[x,names(pr2)[3:37]])})
w2[is.na(w2)] <- 0
##
w3 <- sapply(1:nrow(pr3),function(x){max(pr3[x,names(pr3)[3:37]])})
w3[is.na(w3)] <- 0
##
w4 <- sapply(1:nrow(pr4),function(x){max(pr4[x,names(pr4)[3:37]])})
w4[is.na(w4)] <- 0
w5 <- w1+w2+w3+w4
...
for (i in 1:length(sheetsarea))
{
  ...
  area[,c("NUTS_ID", "CROP_ID", "Average.crop.supply.2001.2020.per.NUTS.1.region..1000.tonnes.")]
...
names(alldays)[c(3,4,6,7)] <- c("heatwave","coldwave","flood","firerisk")
...
for(reg_level in 1:2){
  ...
  for (count in unique(data$country))
  {
    ...
    for(cropid in unique(data$cropid))
    {
      ...
      if(reg_level==1){classifier <- glm(Sub.national.extremes..0.75.perc..~days_heatwave+days_coldwave+days_drought+days_flood+days_firerisk+
                                           exp_area_heatwaves+exp_area_coldwaves+exp_area_droughts+exp_area_floods+exp_area_fireweatherrisk,data=dat)}
      if(reg_level==2){classifier <- glm(National.extreme..0.75.perc..~days_heatwave+days_coldwave+days_drought+days_flood+days_firerisk+
                          exp_area_heatwaves+exp_area_coldwaves+exp_area_droughts+exp_area_floods+exp_area_fireweatherrisk,data=dat)}
